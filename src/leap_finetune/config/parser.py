...
def _validate_parallelism_config(
    ...
    if cp_size > 1 and effective_training_type not in {"sft", "moe_sft"}:
        raise ValueError(
            "context_parallel_size > 1 is currently supported for SFT/MoE SFT only. "
            f"Got training_type='{training_type}'."
        )
    ...
    if cp_size > 1:
        raise ValueError(
            "expert_parallel_size > 1 cannot be combined with "
            "context_parallel_size > 1. Only DP x CP is supported for context "
            "parallelism."
        )
