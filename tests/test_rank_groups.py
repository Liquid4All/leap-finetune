...
from leap_finetune.distribution.data_sharding import (
...
def test_context_parallel_data_config_allows_node_local_noncontiguous_ranks():
    config = ContextParallelDataConfig(context_parallel_size=2)

    output = config.configure(
        datasets={},
        world_size=4,
        worker_handles=None,
        worker_node_ids=["node-a", "node-b", "node-a", "node-b"],
