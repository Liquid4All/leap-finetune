...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=0,
        help=(
            "Abort if more than this many referenced images are missing from "
            "--coco-train2014 (default: 0). RefCOCO/+/g val reference COCO "
            "train2014, so a complete image set drops zero rows; a positive "
            "count means a partial/corrupt COCO that would silently truncate "
            "the eval. Raise only if you knowingly accept a smaller eval set."
        ),
    )
...
def _build_refcoco_variant(
    ...
    *,
    ...
    # A complete COCO train2014 set drops zero rows here (RefCOCO/+/g val all
    # reference train2014 images). A positive count means a partial/corrupt
    # image cache that would silently truncate this eval set — refuse to write
    # it rather than emit non-comparable scores.
    if skipped_missing > max_missing:
        raise SystemExit(
            f"[evals] ABORT: {skipped_missing:,} of "
            f"{skipped_missing + len(rows):,} {hf_id} rows reference images "
            f"missing from {coco_root} (--max-missing={max_missing}). The COCO "
            f"image set is incomplete; writing this jsonl would truncate the "
            f"eval. Re-run prepare_evals.sh (it re-downloads a complete COCO), "
            f"or pass --max-missing to accept a smaller set on purpose."
        )
